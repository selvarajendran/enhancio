import React from 'react';

const GithubItem = (props) => {
    return (
    <div className="itemContainer displayFlex"> 
        <div className="itemLeft">
            <img width="50px" height="50px" src={props.avatar_url} />
        </div>
        <div className="itemRight paddingLeft">
            <div className="itemHeader paddingTop displayFlex">
                <div className="itemTitle">
                    <a href={props.html_url} target="_blank">{props.login}</a>
                </div>
                <div className="itemSubtitle paddingLeft">
                    {props.name || "Unavailable"}
                </div>
            </div>
            <div className="itemDescription paddingTop">
                {props.description || props.company || "Description not found"}
            </div>
            <div className="itemFooter paddingTop displayFlex">
                <div className="itemLocation">
                    <img src="https://cdn2.iconfinder.com/data/icons/navigation-location/512/Gps_locate_location_map_marker_navigate_navigation_pin_plan_road_route_travel_icon_-512.png" width="20px" height="20px"/>
                    <span className="verticalAlign">{props.location || "Location Not found"}</span>
                </div>
                <div className="itemEmail paddingLeft">
                    <img src="data:image/png;base64," width="20px" height="20px"/>
                    <span className="verticalAlign"> {props.email || "Email Not found"} </span>
                </div>
            </div>
        </div>
    </div>
    );
}

export default GithubItem;